...
thread_local! {
    ...
    static ENTRIES: RefCell<Option<&'static Vec<Entry>>> = const { RefCell::new(None) };
}
...
pub fn pick() -> Result<Pick, String> {
    ...
    unsafe {
        ...
        ENTRIES.with(|e| *e.borrow_mut() = Some(entries));
        ...
        let hwnd = CreateWindowExW(
            ...
            WINDOW_STYLE(WS_OVERLAPPEDWINDOW.0 & !WS_MAXIMIZEBOX.0 & !WS_THICKFRAME.0),
            ((sw - w) / 2) as i32,
            ((sh - h) / 2) as i32,
            w,
            h,
        ...
        CreateWindowExW(
            ...
            WINDOW_STYLE((WS_CHILD | WS_VISIBLE).0 | BS_DEFPUSHBUTTON as u32),
            w - 150,
            h - 58,
            120,
            34,
    ...
    match msg {
        ...
        WM_COMMAND => {
            ...
            if code == BN_CLICKED && from == 10 || code == LBN_DBLCLK && from == 9 {
                let lb = match GetDlgItem(Some(hwnd), 9) {
                    Ok(lb) => lb,
